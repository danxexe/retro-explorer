...
pub fn handle_rex_request<R: Runtime>(
    ...
    if asset_key == "favicon.ico" {
        return app_handle.asset_resolver().get("favicon.ico".into())
            .map(|a| tauri::http::Response::builder().header("Content-Type", "image/x-icon").body(a.bytes).unwrap())
    ...
    match app_handle.asset_resolver().get(asset_key.to_string()) {
        Some(asset) => tauri::http::Response::builder()
            ...
            .body(asset.bytes)
            .unwrap(),
        None => tauri::http::Response::builder().status(404).body(vec![]).unwrap(),
    }
}
...
fn serve_transparent_pixel() -> tauri::http::Response<Vec<u8>> {
    let pixel = vec![0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A, 0x00, 0x00, 0x00, 0x0D, 0x49, 0x48, 0x44, 0x52, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x08, 0x06, 0x00, 0x00, 0x00, 0x1F, 0x15, 0xC4, 0x89, 0x00, 0x00, 0x00, 0x0A, 0x49, 0x44, 0x41, 0x54, 0x78, 0x9C, 0x63, 0x00, 0x01, 0x00, 0x00, 0x05, 0x00, 0x01, 0x0D, 0x0A, 0x2D, 0xB4, 0x00, 0x00, 0x00, 0x00, 0x49, 0x45, 0x4E, 0x44, 0xAE, 0x42, 0x60, 0x82];
    tauri::http::Response::builder().header("Content-Type", "image/png").body(pixel).unwrap()
}
